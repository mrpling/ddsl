...
An implementation conforms to DDSL v0.4.0 if it:
...
Single-expression mode (without document features) is a valid subset. An implementation that only supports single expressions (no variables, no multi-line, no comments) conforms to DDSL v0.4.0 expression-level conformance but not document-level conformance.
...
These features are intentionally excluded from DDSL v0.4.0 to preserve simplicity and stability.
